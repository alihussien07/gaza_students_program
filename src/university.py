...
class University:
    ...
    def get_average_grade(self, semester, major=None):
        active_students = [s for s in self.students if not semester.is_student_inactive(s)]
        if major:
            active_students = [s for s in active_students if s.major == major]

        total_grades = sum(s.academic_average for s in active_students)
        average_grade = total_grades / len(active_students) if active_students else 0
        return average_grade

    def get_high_achievers(self):
        high_achievers = [s for s in self.students if s.academic_average > 90]
        return high_achievers
    ...
    def get_top_courses(self):
        ...
        return [course[0] for course in top_courses]
 """